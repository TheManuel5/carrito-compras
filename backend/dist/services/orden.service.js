...
exports.cambiarEstadoOrden = exports.obtenerOrden = exports.listarOrdenes = exports.crearOrden = void 0;
 ...
 */
const crearOrden = async (usuarioId, data) => {
    let itemsParaOrden = [];
    let carritoId;
    if (data.carrito_id) {
        const carrito = await prisma_1.default.ord_carritos.findFirst({
            where: { id: data.carrito_id, usuario_id: usuarioId, estado: 'activo' },
            include: { items: { include: { producto: { include: { stock: true } } } } },
        });
        if (!carrito || carrito.items.length === 0)
            throw new errorHandler_1.AppError('Carrito no encontrado o vacío', 404);
        ...
        for (const item of data.items) {
            ...
            if (!producto)
                throw new errorHandler_1.AppError(`Producto ${item.producto_id} no encontrado`, 404);
            itemsParaOrden.push({
        ...
        throw new errorHandler_1.AppError('Debe proporcionar items o carrito_id', 400);
    }
    const metodoEnvio = await prisma_1.default.ord_metodos_envio.findUnique({ where: { id: data.metodo_envio_id } });
    if (!metodoEnvio)
        throw new errorHandler_1.AppError('Método de envío no disponible', 404);
    const orden = await prisma_1.default.$transaction(async (tx) => {
        ...
        for (const item of itemsParaOrden) {
            ...
            if (disponible < item.cantidad)
                throw new errorHandler_1.AppError(`Stock insuficiente para: ${item.nombre}`, 409);
        ...
        if (carritoId)
            await tx.ord_carritos.update({ where: { id: carritoId }, data: { estado: 'completado' } });
 ...
 */
const listarOrdenes = async (filtros, usuarioId) => {
    ...
    if (fecha_inicio || fecha_fin) {
        where.created_at = {};
        if (fecha_inicio)
            where.created_at.gte = new Date(fecha_inicio);
        if (fecha_fin)
            where.created_at.lte = new Date(fecha_fin);
    }
    if (monto_min !== undefined)
        where.total = { ...where.total, gte: monto_min };
    if (monto_max !== undefined)
        where.total = { ...where.total, lte: monto_max };
    if (search)
        where.numero_orden = { contains: search, mode: 'insensitive' };
    const [total, ordenes] = await Promise.all([
        prisma_1.default.ord_ordenes.count({ where }),
        prisma_1.default.ord_ordenes.findMany({
            where,
            skip,
            take: limit,
...
exports.listarOrdenes = listarOrdenes;
/**
 * Obtener orden por ID
 */
const obtenerOrden = async (id, usuarioId) => {
    const where = { id };
    if (usuarioId)
        where.usuario_id = usuarioId;
    const orden = await prisma_1.default.ord_ordenes.findFirst({
        where,
        include: {
 ...
 */
const cambiarEstadoOrden = async (id, estado, comentario, tracking, usuarioId) => {
    const orden = await prisma_1.default.ord_ordenes.findUnique({ where: { id } });
    if (!orden)
        throw new errorHandler_1.AppError('Orden no encontrada', 404);
    const ordenActualizada = await prisma_1.default.$transaction(async (tx) => {
...
exports.cambiarEstadoOrden = cambiarEstadoOrden;
