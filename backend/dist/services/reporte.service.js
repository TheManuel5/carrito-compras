...
exports.analisisRFM = exports.analisisABC = exports.obtenerDatosGraficos = exports.obtenerKpis = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
/**
 * Estadísticas para el Dashboard
 */
const obtenerKpis = async (fechaInicio, fechaFin) => {
    const hoy = new Date();
    const inicio = fechaInicio || new Date(hoy.getFullYear(), hoy.getMonth(), 1);
    const fin = fechaFin || hoy;
    const [totalOrdenes, totalVentas, ordenesEstados, productosAgotados, productosStockBajo, clientesNuevos, ordenesPendientes, carritosTotales, ordenesCompletadas, ingresosPorCategoria,] = await Promise.all([
        prisma_1.default.ord_ordenes.count({ where: { created_at: { gte: inicio, lte: fin } } }),
        prisma_1.default.ord_ordenes.aggregate({
            where: {
        ...
        prisma_1.default.ord_ordenes.groupBy({
            by: ['estado'],
            _count: { id: true },
        }),
        prisma_1.default.inv_stock_producto.count({ where: { cantidad: 0 } }),
        prisma_1.default.inv_stock_producto.count({
            where: { cantidad: { gt: 0 }, AND: [{ cantidad: { lte: prisma_1.default.inv_stock_producto.fields.cantidad_min } }] },
        }),
        prisma_1.default.seg_usuarios.count({ where: { created_at: { gte: inicio, lte: fin } } }),
        prisma_1.default.ord_ordenes.count({ where: { estado: 'pendiente_pago' } }),
        prisma_1.default.ord_carritos.count({ where: { created_at: { gte: inicio, lte: fin } } }),
        prisma_1.default.ord_ordenes.count({
...
exports.obtenerKpis = obtenerKpis;
/**
 * Datos para gráficos del dashboard
 */
const obtenerDatosGraficos = async (periodo = 'mes') => {
...
exports.obtenerDatosGraficos = obtenerDatosGraficos;
/**
 * Análisis ABC de productos
 */
const analisisABC = async () => {
    const productos = await prisma_1.default.$queryRaw `
  ...
  `;
    const totalIngresos = productos.reduce((acc, p) => acc + Number(p.ingresos), 0);
    let acumulado = 0;
...
exports.analisisABC = analisisABC;
/**
 * Análisis RFM de clientes
 */
const analisisRFM = async () => {
    const clientes = await prisma_1.default.$queryRaw `
    SELECT
  ...
  `;
    ...
    return clientes.map((c) => {
        const diasDesdeUltimaCompra = c.ultima_compra
            ...
            : 9999;
        const segmento = diasDesdeUltimaCompra <= 30 && Number(c.frecuencia) >= 3 ? 'vip' :
...
exports.analisisRFM = analisisRFM;
