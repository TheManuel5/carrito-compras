...
exports.generarFactura = exports.generarReporteInventario = exports.generarReporteOrdenes = void 0;
const pdfkit_1 = __importDefault(require("pdfkit"));
/**
 * Genera el encabezado corporativo del PDF
 */
const encabezado = (doc, opts) => {
...
const piePagina = (doc) => {
    ...
    for (let i = 0; i < pageCount; i++) {
        ...
        doc.fontSize(8).fillColor('#94a3b8')
            .text(`Página ${i + 1} de ${pageCount} | ${new Date().toLocaleDateString('es-PE')}`, 50, doc.page.height - 40, { align: 'center' });
    }
};
/**
 * Tabla de datos
 */
const tabla = (doc, encabezados, filas, anchos) => {
    const startX = 50;
    let y = doc.y;
    const rowHeight = 22;
    // Encabezado
 ...
 */
const generarReporteOrdenes = async (ordenes, res, filtros = '') => {
    ...
    doc.moveDown();
    tabla(doc, ['Nro. Orden', 'Fecha', 'Cliente', 'Estado', 'Items', 'Total'], ordenes.map((o) => [
        o.numero_orden,
...
exports.generarReporteOrdenes = generarReporteOrdenes;
/**
 * Reporte de inventario valorizado
 */
const generarReporteInventario = async (productos, res) => {
    const doc = new pdfkit_1.default({ margin: 50, bufferPages: true });
    res.setHeader('Content-Type', 'application/pdf');
    ...
    const totalValor = productos.reduce((acc, p) => acc + (p.stock?.cantidad || 0) * Number(p.precio_costo || 0), 0);
    doc.fillColor('#1e293b').fontSize(10).font('Helvetica-Bold')
        .text(`Valor total de inventario: S/. ${totalValor.toFixed(2)}`);
    doc.moveDown();
    tabla(doc, ['SKU', 'Producto', 'Categoría', 'Stock', 'P. Costo', 'Valor Total'], productos.map((p) => [
        p.sku,
        p.nombre,
        p.categoria?.nombre || '',
...
exports.generarReporteInventario = generarReporteInventario;
/**
 * Factura individual por orden
 */
const generarFactura = async (orden, res) => {
    ...
    doc.moveDown();
    // Items
    tabla(doc, ['Producto', 'SKU', 'Cant.', 'P. Unit.', 'Subtotal'], orden.items.map((item) => [
